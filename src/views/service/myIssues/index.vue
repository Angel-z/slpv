<template>
  ...
  <h1 style="text-align: center">我的问题</h1>
      ...
      <el-row v-for="(item, index) in issueList" :key="index">
        <el-col>
          <router-link target="_blank" :to="{path: '/issue/' + item.issueId}">
            <el-card class="content-card" shadow="hover" :body-style="{ 'padding-bottom': '0 !important'}">
              <template v-slot:header>
                ...
                <div class="subTitle">
                  <el-space>
                    ...
                    <span>创建日期：{{ parseTime(item.createTime, '{y}年{m}月{d}日 {h}:{i}') }}</span>
...
import {getMyIssues, listIssue} from "@/api/slp/issue.js";
